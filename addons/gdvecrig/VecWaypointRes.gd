...
func edit_weight(bone: Bone2D, plugin: GDVecRig, weights: Dictionary[Bone2D, float]):
	if bone == null:
		print("refusing to edit no bone")
		return
	var w = get_weight(bone, weights)
...
func add_weight(bone: Bone2D, weight, weights: Dictionary[Bone2D, float]):
	if bone == null:
		print("refusing to edit no bone")
		return
	var w = get_weight(bone, weights)
...
func compute_bone_transform(bone: Bone2D, cache: Dictionary, weights: Dictionary[Bone2D, float]) -> Transform2D:
	...
	if p_bone != null:
		p_transform = compute_bone_transform(p_bone, cache, weights)
	elif p != null:
...
func compute_value(skeleton: Skeleton2D, transform_cache: Dictionary, weights: Dictionary[Bone2D, float]):
	...
	for i in weights.keys():
		...
		var tformed = compute_bone_transform(bone, transform_cache, weights) * value
